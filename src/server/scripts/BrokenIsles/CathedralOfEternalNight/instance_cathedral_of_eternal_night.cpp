        ...
        void OnUnitDeath(Unit* creature) override
        {
            switch (creature->GetEntry())
            {
                case NPC_GAZERAX:
                    ...
                    {
                        thrashbite->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                ...
                case NPC_BATS_INTRO:
                {
                    ...
                    if (deads_npc >= 4)
                    {
                        if (TempSummon* illidan = instance->SummonCreature(NPC_ILLIDAN_INTRO, { -600.66f, 2527.25f, 532.47f, 0.02f }))
                        {
        ...
        bool SetBossState(uint32 type, EncounterState state) override
        {
            ...
            switch (type)
            {
                    ...
                    switch (state)
                    {
                        case NOT_STARTED:
                            if (Creature* egida = instance->GetCreature(EgidaDomatraxStarterGuid))
                            {
                                ...
                                egida->SetFlag(UNIT_FIELD_FLAGS, UNIT_NPC_FLAG_GOSSIP);
                        ...
                        case DONE:
                            for (auto guid : introMephistrothAdds)
                                if (Creature* creature = instance->GetCreature(guid))
                                {
                                    if (creature->GetEntry() == NPC_BATS_FLY && instance->GetDifficultyID() == DIFFICULTY_HEROIC)
                                        continue;
                ...
                case DATA_MEPHISTROTH:
                    ...
                    if (state == DONE)
                    {
                        if (TempSummon* illidan = instance->SummonCreature(NPC_ILLIDAN_INTRO, { -547.93f, 2532.18f, 533.94f, 5.01f }))
                            illidan->CastSpell(illidan, 239484);

                        if (TempSummon* maiev = instance->SummonCreature(NPC_MAIEV_INTRO, { -579.73f, 2526.37f, 533.98f, 0.00f }))
                            maiev->GetMotionMaster()->MovePoint(0, -553.83f, 2518.90f, 533.94f);

                        if (TempSummon* egvin = instance->SummonCreature(NPC_EGVIN_FINAL, { -546.11f, 2523.09f, 534.40f, 2.96f }))
                        {
