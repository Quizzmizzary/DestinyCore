...
void WildBattlePetMgr::Load()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT Zone, Species, `BattlePetEntry`, `Max`, RespawnTime, MinLevel, MaxLevel, CreatureEntry FROM wild_battlepet_zone_pool");
    if (!result)
    {
...
void WildBattlePetMgr::ReplaceCreature(Creature* creature, WildPetPoolTemplate* wTemplate, WildBattlePetPool* pTemplate)
{
    ...
    // Set creature flags
    replacementCreature->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_WILD_BATTLE_PET);
...
void WildBattlePetMgr::EnableWildBattle(Creature* creature)
{
    ...
    if (AreaTableEntry const* areaInfo = sAreaTableStore.LookupEntry(creature->GetZoneId()))
    {
...
void WildBattlePetMgr::Depopulate(WildBattlePetPool* pTemplate)
{
    ...
    {
        Unit* unit = ObjectAccessor::FindCreature(creature.first);
...
void WildBattlePetMgr::UnreplaceCreature(Creature* creature)
{
    ...
    Unit* unit = ObjectAccessor::FindCreature(pTemplate->ReplacedRelation[creature->GetGUID()]);
    if (!unit || !unit->ToCreature())
        return;
...
void WildBattlePetMgr::LeaveBattle(Creature* creature, bool /*p_Defeated*/)
{
    ...
    Unit* unit = ObjectAccessor::FindCreature(creature->replacementFromGUID);
    if (!unit || !unit->ToCreature())
        return;
