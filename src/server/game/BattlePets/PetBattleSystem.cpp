...
eBattlePetRequests PetBattleSystem::CanPlayerEnterInPetBattle(Player* player, PetBattleRequest* petBattleRequest)
{
   ...
   /* // Check positions
    for (const auto& itr : petBattleRequest->TeamPosition)
        if (player->GetMap()->getObjectHitPos(player->GetPhaseShift(), petBattleRequest->PetBattleCenterPosition.m_positionX, petBattleRequest->PetBattleCenterPosition.GetPositionY, petBattleRequest->PetBattleCenterPosition.m_positionZ, itr.m_positionX, itr.m_positionY, itr.m_positionZ, 1.0f))
