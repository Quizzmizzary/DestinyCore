...
bool PetBattleTeam::Update()
{
    ...
    uint32 thisTeamID = PetBattleInstance->Teams[0] == this ? PETBATTLE_TEAM_1 : PETBATTLE_TEAM_2;
...
void PetBattle::Begin()
{
    ...
    if (BattleType == PETBATTLE_TYPE_PVE)
    {
        ...
        if (Player* player = ObjectAccessor::FindPlayer(Teams[PETBATTLE_TEAM_1]->OwnerGuid))
        {
    ...
    {
        Player* playerA = ObjectAccessor::FindPlayer(Teams[PETBATTLE_TEAM_1]->OwnerGuid);
        Player* playerB = ObjectAccessor::FindPlayer(Teams[PETBATTLE_TEAM_2]->OwnerGuid);
...
void PetBattle::ProceedRound()
{
    ...
    for (auto& team : Teams)
    {
        ...
        if (auto player = ObjectAccessor::FindConnectedPlayer(team->PlayerGuid))
...
void PetBattle::Finish(uint32 winnerTeamID, bool aborted, bool ignoreAbandonPenalty)
{
    ...
    for (size_t currentTeamID = 0; currentTeamID < MAX_PETBATTLE_TEAM; ++currentTeamID)
    {
        ...
        {
            auto player = ObjectAccessor::FindConnectedPlayer(Teams[currentTeamID]->PlayerGuid);
            if (!player)
                continue;
            ...
            if (BattleType == PETBATTLE_TYPE_PVE && currentTeamID == winnerTeamID && Teams[currentTeamID]->CapturedPet != PETBATTLE_NULL_ID)
            {
                ...
                {
                    player->UpdateCriteria(CRITERIA_TYPE_ADD_BATTLE_PET_JOURNAL, speciesInfo->CreatureID);
                    player->UpdateCriteria(CRITERIA_TYPE_CAPTURE_SPECIFIC_BATTLEPET, speciesInfo->ID, 0, speciesInfo->ID);
                    player->UpdateCriteria(CRITERIA_TYPE_CAPTURE_PET_IN_BATTLE, 1, Pets[Teams[currentTeamID]->CapturedPet]->Quality, speciesInfo->ID);
                }
    ...
    for (size_t currentTeamID = 0; currentTeamID < MAX_PETBATTLE_TEAM; ++currentTeamID)
    {
        ...
        {
            auto player = ObjectAccessor::FindPlayer(Teams[currentTeamID]->PlayerGuid);
            if (!player)
                continue;
            ...
            player->GetScheduler().Schedule(Seconds(10), Seconds(11), [](TaskContext context)
            {
                Player* plr = GetContextPlayer();
                plr->SummonLastSummonedBattlePet();
            ...
            if (BattleType == PETBATTLE_TYPE_PVP_MATCHMAKING)
                player->TeleportTo(player->m_recall_location);
        }
        else
        {
            if (BattleType == PETBATTLE_TYPE_PVE && PveBattleType == PVE_PETBATTLE_WILD)
            {
                for (size_t i = 0; i < Teams[currentTeamID]->TeamPetCount; ++i)
                {
                    ...
                    auto wildPet = ObjectAccessor::FindCreature(currentPet->OriginalCreature);
                    if (!wildPet)
                        continue;
